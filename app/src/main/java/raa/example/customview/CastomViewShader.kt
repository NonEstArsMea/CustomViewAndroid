...
class CastomViewShader @JvmOverloads constructor(
    context: Context,
    ...
    init {
        if (attr != null) {
            val ta: TypedArray = context.obtainStyledAttributes(attr, R.styleable.CastomViewShader)
            borderWidth = ta.getDimension(
                R.styleable.CastomViewShader_borderWidth_shader,
                context.dpToPx(DEFAULTS_BORDER_WIDTH)
            )

            borderColor = ta.getColor(
                R.styleable.CastomViewShader_borderColor_shader,
                DEFAULTS_BORDER_COLOR
            )

            initials = ta.getString(R.styleable.CastomViewShader_initials_shader) ?: "??"
            ta.recycle()
        }
    ...
    companion object {
        ...
        private const val DEFAULTS_BORDER_COLOR = Color.RED
